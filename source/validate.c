	...
	while (fgets(line, sizeof(line), ifp)) /* pass line on the file */ 
	{
		...
		while (word != NULL) 
		{
			if ((strchr(word, ':') != NULL))/*if this is label*/ 
			{
				...
				if((word[0] == '*') && (word[1] == 'r') && (word[2] >= '0') && (word[2] <= '7') && (strlen(word) == 3))/*If it is a * register it is an invalid label and we will return an error*/
				{
...
void is_valid_number(char *str, int line_number) 
{
	...
	{
		if((atoi(number) < -16384)  ||  (atoi(number) > 16383))/*check if the numer is legal*/
		{
...
int is_register(char* word)/*chaking the register*/ 
{
	...
	if(((word[0] != ' ') && (word[0] != 'r')) || ((word[0] != ' ') && (word[0] != '*') && (word[1] != 'r')))/*if there is no 'r' in first place*/
	{
		return 0;	
		
	}
	else if(((word[0] == ' ') && (word[1] != 'r')) || ((word[0] == ' ') && (word[1] != '*')  && (word[2] != 'r')))/*if there is no 'r' in first place*/
	{
		return 0;
	}
	for (i = 0; i < len; i++) 
...
int is_valid_label(char* word) 
{
	...
	if(word[0] == '*')
	{
		if(isdigit(word[1]))/*if isn't letter in first palace*/
		{
			return 0;
		}
	...
	while (fgets(line, sizeof(line), ifp)) 
	{
		...
		for(i = 0; i < 16; i++)
		{
			if(strstr(line, opcode[i].opName))
			{
				...
				else if(i == 5 || i == 6 || i == 7 || i == 8 || i == 11)/*not, red, dec, clr, inc*/
				{
...
void is_valid_mov(char * str, int line_num) 
{
	...
	if(check0123(word, line_num) == 0)/*check source operand*/
	{
		return;
	}
	while (word != NULL) 
	{
		...
		if (word != NULL) 
		{
			...
			if(check123(word, line_num) == 0)/*check second operand*/
			{
				return;
			}
...
void is_valid_jmp(char * str, int line_num) 
{
	...
	else
	{
		while (word != NULL) 
		{
			if(check123(word, line_num) == 0)/*check operand*/
			{
				return;
			}
			word = strtok(NULL, " ,\n");
...
int check0123(char * word, int line_num)
 {
	...
	else if((word[0] == 'r') || ((word[0] == '*') && (word[1] == 'r')))/*check if operand is register*/
	{
...
int check123(char * word, int line_num)
{
	...
	else if((word[0] == 'r') || ((word[0] == '*') && (word[1] == 'r')))/*if in the first word is register*/
	{
		...
		if(is_valid_label(cp) == 1)
		{
			...
			else/*if the label is legal*/
			{
				...
				if(cp != NULL)
				{
					...
					if(check0123(cp, line_num) == 0)/*check first parameter*/
					{
						return;
					}
					cp = strtok(NULL,")\n");
					if(cp != NULL)
					{
						...
						if(check0123(cp, line_num) == 0)/*check second parameter*/
						{
							return;
						}
					}
